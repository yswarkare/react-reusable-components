export const constructor = (array, model_01) => {
	super(array);
...
export const setNewObjectList = (array, model_01) => {
	if (Array.isArray?.(array)) {
		this.forEach(() => {
			this.pop();
		});
...
export const findOneByKeyValue = (object) => {
	let result = [];
...
export const newArray = (prevArray, newArray, objectModel) => {
	newArray.map(() => newArray.pop());
	for (let i = 0; i < prevArray.length; i++) {
		if (objectModel) {
			const element = new objectModel();
			element.newObject(prevArray[i]);
			newArray.push(element);
		} else {
			const element = prevArray[i];
			newArray.push(element);
		}
	}
	return newArray;
...
export const updateByID = (element) => {
	for (let i = 0; i < this.length; i++) {
...
export const updateByIndex = (element, index) => {
...
export const updateKeyValueByID = (object) => {
...
export const updateKeyValueByIndex = (object) => {
	if (object.key === "delete") {
		this.splice(object.index, 1);
	} else if (object.key === "add-new-at-start") {
		this.unshift({});
...
export const deleteByID = (object) => {
	for (let i = 0; i < this.length; i++) {
		if (this[i].id === object.id) {
			this.splice(i, 1);
		}
...
export const searchStrInArrOfObjects = (object) => {
...
export const isOnlyElement = (index) => {
	return this[index] === 0 && this.length === 1 ? true : false;
};

export const hasOnlyOneElement = () => {
	return this.length === 1 ? true : false;
};

export const isLastElement = (index) => {
